...
describe("toolTextResult", () => {
  it("wraps text as a non-error result", () => {
    expect(toolTextResult("hello")).toEqual({
...
describe("toolErrorResult", () => {
  it("includes the HTTP status and response body snippet for a GovApiError", () => {
    const error = new GovApiError(
      "GET https://example.test returned HTTP 401",
      {
  ...
  it("omits the body suffix when the GovApiError has no body", () => {
    const error = new GovApiError("Request timed out", {
      url: "https://example.test",
    });
    ...
    expect(result.content[0].text).toBe(
      "Failed to fetch thing: Request timed out",
    );
  });
  ...
  it("truncates long response bodies", () => {
    ...
    const error = new GovApiError(
      "GET https://example.test returned HTTP 500",
      {
        url: "https://example.test",
        status: 500,
        body: longBody,
