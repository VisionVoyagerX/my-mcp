...
**CitizenMCP**: A public Cloudflare Worker exposing Greek government transparency and open data — Diavgeia, data.gov.gr, and ΓΕΜΗ — as an MCP server.

## Overview

This package deploys the Diavgeia, CKAN (data.gov.gr), and ΓΕΜΗ tool sets from `@my-mcp/core` to Cloudflare Workers, making them available as a remote MCP server that any MCP client can point at via HTTP. Diavgeia and CKAN need no accounts or API keys (an optional free `DATA_GOV_GR_TOKEN` just raises CKAN's rate limit); ΓΕΜΗ needs a free server-side API key, same as on business-mcp — see Auth below.
...
## Auth — Diavgeia/CKAN open, ΓΕΜΗ needs a server key

- **Diavgeia / CKAN**: fully open, no key.
...
4. Click **Add** — no OAuth or API key prompt appears; ΓΕΜΗ's key is held server-side, same as Diavgeia/CKAN needing none at all from the client's perspective.
...
- [Diavgeia Client](/packages/core/src/diavgeia/), [CKAN Client](/packages/core/src/ckan/), [ΓΕΜΗ Client](/packages/core/src/gemi/) — shared client libraries
